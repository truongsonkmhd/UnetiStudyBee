...
import com.truongsonkmhd.unetistudy.sevice.AiLearningPathService;
...
public class AiLearningPathServiceImpl implements AiLearningPathService {
    ...
    public List<AiLessonSuggestionDTO> suggestNextLessons(UUID userId, String courseSlug) {
        ...
        Map<UUID, LessonProgress> progressMap =
                lessonProgressRepository.findByUserAndCourse(userId, course.getCourseId())
                        .stream()
        ...
        for (CourseModule module : course.getModules()) {
            for (CourseLesson lesson : module.getLessons()) {
                ...
                boolean prereqDone = lessonPrerequisiteRepository
                        ...
                        .stream()
                        .allMatch(p ->
                                progressMap.containsKey(p.getPrerequisiteLesson().getLessonId()) && progressMap.get(p.getPrerequisiteLesson().getLessonId()).getStatus() == ProgressStatus.NOT_STARTED
                        );
