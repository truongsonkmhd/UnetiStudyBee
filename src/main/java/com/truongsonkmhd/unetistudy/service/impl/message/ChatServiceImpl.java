package com.truongsonkmhd.unetistudy.service.impl.message;
...
public class ChatServiceImpl implements ChatService {
    ...
    public ChatRoom createOrGetDirect(UUID me, UUID target) {
        if (target == null || me.equals(target))
    ...
    public ChatRoom createGroup(UUID me, String name, List<UUID> memberIds) {
        ...
        for (UUID u : set) {
            if (!u.equals(me))
                upsertMember(room.getId(), u, ChatRoomMember.Role.MEMBER);
        }
        return room;
    }
    ...
    public List<ChatMessage> listMessages(UUID me, UUID roomId, Instant before, int size) {
        ...
        if (before == null)
            return msgRepo.findLatest(roomId, p);
    ...
    private void upsertMember(UUID roomId, UUID userId, ChatRoomMember.Role role) {
        ...
        ChatRoomMember m = memberRepo.findById(id)
                .orElseGet(() -> ChatRoomMember.builder().id(id).muted(false).build());
        m.setRole(role);
        memberRepo.save(m);
    }
}
